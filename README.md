...
graph TB
    ...
    %% 响应处理阶段
    subgraph 响应处理阶段
        O -->|2xx成功| P{Content-Encoding检查}
        O -->|错误响应| Q[处理错误并记录日志]
        O -->|超时| R[处理超时错误]
        O -->|网络错误| S[处理网络错误]
        
        P -->|gzip| T[Gunzip解压]
        P -->|deflate| U[Inflate解压]
        P -->|其他| V[直接处理]
        ...
        subgraph 流式处理
            ...
            W --> X[Transform流处理]
            X -- 数据收集 --> Y[收集响应数据]
            X -- Token计算 --> Z[计算Completion Tokens]
            X -- 日志记录 --> AA[记录响应日志]
            Z --> AB[更新Token统计]
        end
    ...
    style Z fill:#FFE4B5,stroke:#333
    style AA fill:#FFE4B5,stroke:#333
    style AB fill:#FFE4B5,stroke:#333
    
    classDef async fill:#FFE4B5,stroke:#333;
    classDef phase fill:#f0f8ff,stroke:#333,stroke-width:2px;
    classDef stream fill:#e6f3ff,stroke:#333;
    
    class M,Z,AA,AB async;
    class 请求处理阶段,请求转发阶段,响应处理阶段 phase;
    class 流式处理 stream;
    
    %% 图例说明
    subgraph 图例
        asyncLegend[异步处理]:::async
        phaseLegend[处理阶段]:::phase
        streamLegend[流式处理]:::stream
    end
```
